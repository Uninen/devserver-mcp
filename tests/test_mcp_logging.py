...
async def test_mcp_tool_calls_logged_to_tui(manager):
    ...
    async def start_server_with_logging(name: str) -> dict:
        await manager._notify_log(
            "MCP Server", "12:00:00", f"Tool 'start_server' called with: {{'name': {repr(name)}}}"
        )
        return await manager.start_server(name)

    async def stop_server_with_logging(name: str) -> dict:
        await manager._notify_log("MCP Server", "12:00:00", f"Tool 'stop_server' called with: {{'name': {repr(name)}}}")
        return await manager.stop_server(name)

    async def get_server_status_with_logging(name: str) -> dict:
        await manager._notify_log(
            "MCP Server", "12:00:00", f"Tool 'get_server_status' called with: {{'name': {repr(name)}}}"
        )
        return manager.get_server_status(name)

    async def get_server_logs_with_logging(name: str, lines: int = 500) -> dict:
        await manager._notify_log(
            "MCP Server", "12:00:00", f"Tool 'get_server_logs' called with: {{'name': {repr(name)}, 'lines': {lines}}}"
        )
...
async def test_mcp_logging_doesnt_leak_to_terminal(manager):
    """Test that MCP logging goes only to TUI callbacks, not to stdout/stderr"""
    # Capture stdout and stderr
    stdout_capture = io.StringIO()
    stderr_capture = io.StringIO()

    with patch("sys.stdout", stdout_capture), patch("sys.stderr", stderr_capture):
        # Set up a log callback to capture TUI messages
        logged_messages = []
        ...
        # Log MCP server startup (this is what happens in TUI on_mount)
        await manager._notify_log(
            "MCP Server", "12:00:00", "MCP Server started at http://localhost:3001/mcp/ (streamable-http transport)"
        )

        # Simulate tool calls being logged
    ...
    # Check that messages were captured by TUI callback
    assert len(logged_messages) == 3, f"Expected 3 logged messages, got {len(logged_messages)}"

    # Check that no output leaked to stdout or stderr
    ...
    # Verify the messages were properly captured
