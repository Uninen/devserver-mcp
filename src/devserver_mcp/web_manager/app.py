...
from devserver_mcp.web_manager.auth import security
...
async def verify_token(
    ...
    if not deps.bearer_token or credentials.credentials != deps.bearer_token:
...
def setup_routes(app: FastAPI) -> None:
    ...
    async def get_projects(_: Annotated[None, Depends(verify_token)], deps: Dependencies = Depends(get_deps)):  # noqa: B008
    ...
    async def register_project(
        project: Project, _: Annotated[None, Depends(verify_token)], deps: Dependencies = Depends(get_deps)  # noqa: B008
    ):
    ...
    async def get_project_servers(
        project_id: str, _: Annotated[None, Depends(verify_token)], deps: Dependencies = Depends(get_deps)  # noqa: B008
    ):
